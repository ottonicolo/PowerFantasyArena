...
export default function Home() {
  ...
  return <main className="shell decadeShell">
    ...
    {view === "method" && <section className="methodPage"><div className="methodHero"><span>HOW THE HARNESS WORKS</span><h1>{t.methodTitle}</h1><p>{t.methodIntro}</p></div><div className="ruleGrid"><article><b>120</b><h3>{t.eventPool}</h3><p>{locale === "zh" ? "30 个网文母题，各分四档强度。光环改变好运、坏事和混合事件的抽取权重。" : "Thirty web-fiction motifs, each with four intensity tiers. Halo changes the draw weights for fortune, peril, and mixed events."}</p></article><article><b>40 / 80 / 120</b><h3>{t.encounterCheck}</h3><p>{locale === "zh" ? "前期看运气和距离，中期基础概率上升，最后一个月所有存活者必定会合。" : "Early meetings depend on luck and distance. Midgame odds rise. Every survivor converges in month 120."}</p></article><article><b>6</b><h3>{t.checkpoints}</h3><p>{t.checkpointNote}</p></article><article><b>∞</b><h3>LOSSLESS RUN FOLDER</h3><p>{locale === "zh" ? "每局都有自己的文件夹，保存 Markdown 报告和完整 JSON。" : "Every run gets its own folder with a Markdown report and the full JSON trace."}</p></article></div><div className="ethicsNote"><b>DECISION RECORD</b><p>{t.safety}</p></div></section>}
  </main>;
}
